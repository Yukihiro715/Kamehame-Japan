...
const LEGAL_JA: LegalCopy = {
  ...
  rows: [
    ...
    ["商品代金以外に必要な料金", "銀行振込の場合の振込手数料、およびインターネット接続にかかる通信料はお客様のご負担です。追加オプション(芸妓・舞妓の追加など)の料金は各体験ページに記載しています。"],
    ["お支払方法", "クレジットカード、または銀行振込"],
    ["お支払時期", "ご予約確定後、当社がご案内する期日までにお支払いください。期日までにお支払いが確認できない場合、ご予約を取り消すことがあります。"],
...
const LEGAL_EN: LegalCopy = {
  ...
  rows: [
    ...
    ["Costs beyond the price", "Bank transfer fees and your own internet connection costs. Optional extras (an additional geiko or maiko, for example) are priced on each experience page."],
    ["Payment methods", "Credit card or bank transfer."],
    ["When to pay", "After your booking is confirmed, by the date we give you. If payment has not arrived by then, the booking may be cancelled."],
...
const TERMS_JA: TermsCopy = {
  ...
  sections: [
    ...
    { heading: "2. お申込みと予約の成立", body: [
      ...
      "ご案内に記載の期限内にお客様がその条件に同意した時点で予約が確定し、その時点からキャンセル規定が適用されます。芸妓・舞妓など出演者の手配を伴う体験では、受け入れ先の慣習上、正式な手配を始めた時点で取消料が発生するため、条件のご確認を先にお願いしています。",
    ]},
    { heading: "3. 料金とお支払い", body: [
      "料金は各体験ページに表示する日本円の金額で、消費税・サービス料を含みます。人数・時期により異なります。ご案内した金額以外を当日に請求することはありません。",
      "お支払いはクレジットカードまたは銀行振込により、ご案内する期日までにお願いします。期日までにお支払いが確認できない場合、当社は予約を取り消すことがあります。振込手数料はお客様のご負担です。",
    ]},
...
const TERMS_EN: TermsCopy = {
  ...
  sections: [
    ...
    { heading: "2. Requests and bookings", body: [
      ...
      "A booking exists once you accept those conditions within the time stated in our message, and the cancellation terms apply from that moment. Where a performer such as a geiko or maiko must be secured, the host's custom is that cancellation fees begin once formal arranging starts, which is why we confirm the conditions with you first.",
    ]},
    { heading: "3. Prices and payment", body: [
      ...
      "Payment is by credit card or bank transfer, by the deadline we give you. If payment has not arrived by then we may cancel the booking. Bank transfer fees are yours.",
    ]},
