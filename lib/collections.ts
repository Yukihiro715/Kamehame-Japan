...
const STR: Record<Lang, CollectionStrings> = {
  en: {
    ...
    allLead: "From centuries-old traditions to the culture shaping Japan today. Explore carefully selected experiences — food, craft, art, beauty, sport and special access you won't find in a typical sightseeing itinerary. Choose by city, interest or simply what sounds unforgettable.",
  ...
  es: {
    ...
    allLead: "De tradiciones centenarias a la cultura que da forma al Japón de hoy. Explora experiencias cuidadosamente seleccionadas — comida, oficio, arte, belleza, deporte y accesos especiales que no encontrarás en un itinerario turístico al uso. Elige por ciudad, por interés o simplemente por lo que suene inolvidable.",
  ...
  ja: {
    ...
    allLead: "何百年も続く伝統から、いまの日本をかたちづくる文化まで。食、工芸、芸術、美容、スポーツ、そして通常は入れない場所へ——一般的な観光行程にはない、厳選した体験をご覧ください。都市から、関心から、あるいは「忘れられなさそう」という直感から選んでください。",
  ...
  fr: {
    ...
    allLead: "Des traditions séculaires à la culture qui façonne le Japon d'aujourd'hui. Explorez des expériences soigneusement sélectionnées — cuisine, artisanat, art, beauté, sport et accès privilégiés que vous ne trouverez pas dans un itinéraire touristique classique. Choisissez par ville, par envie, ou simplement ce qui vous semble inoubliable.",
  ...
  "zh-tw": {
    ...
    allLead: "從數百年的傳統，到形塑今日日本的文化。探索精心挑選的體驗——美食、工藝、藝術、美容、運動與一般觀光行程找不到的特別通道。依城市、依興趣，或單純憑「聽起來會難忘」來選。",
